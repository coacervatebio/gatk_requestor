...
def data(alignments_dir: Path) -> Iterator[Task]:
    ...
    for reg_dir in alignments_dir.glob('chr*'):
        for almt in reg_dir.glob('*cram'):
            print(almt)
            sample = almt.with_suffix('')
            print(sample)
            inputs = {
                'align_path': f'{sample}.cram',
                'index_path': f'{sample}.cram.crai',
                'region': reg_dir.name,
                'vcf_path': str(prov_outpath.joinpath(f'{PurePath(sample).name}.vcf.gz'))
            }
...
#         image_hash="55fe5249e030ef60c29d5db7bf2f4484f24205670cceefd89a887cd8",
#         min_mem_gib=1.0,
#         min_storage_gib=2.0,
#     )
